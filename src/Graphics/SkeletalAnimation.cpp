...
NodeChannel::NodeChannel(Node* root, aiNodeAnim* animNode) {
	...
	for (unsigned int i = 0; i < animNode->mNumPositionKeys; i++) {
		...
		positionKeys[i] = std::pair<float, glm::vec3>{(float)aiPositionKeys[i].mTime, val};
	}
	...
	for (unsigned int i = 0; i < animNode->mNumScalingKeys; i++) {
		...
		scalingKeys[i] = std::pair<float, glm::vec3>{(float)aiScalingKeys[i].mTime, val};
	}
	...
	for (unsigned int i = 0; i < animNode->mNumRotationKeys; i++) {
		...
		rotationKeys[i] = std::pair<float, glm::quat>{(float)aiRotationKeys[i].mTime, val};
	}
